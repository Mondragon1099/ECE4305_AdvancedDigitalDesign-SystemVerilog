...
(
   ...
   (.clk(clk),
    ...
    // slot 14: PMOD-CMPS2 i2c 
    chu_i2c_core CMPS2_slot14 
    (.clk(clk),
     .reset(reset),
     .cs(cs_array[`S14_CMPS2]),
     .read(mem_rd_array[`S14_CMPS2]),
     .write(mem_wr_array[`S14_CMPS2]),
     .addr(reg_addr_array[`S14_CMPS2]),
     .rd_data(rd_data_array[`S14_CMPS2]),
     .wr_data(wr_data_array[`S14_CMPS2]),
     .scl(cmps2_scl),
     .sda(cmps2_sda)
     );
